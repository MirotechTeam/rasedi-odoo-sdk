...
class PaymentPortalRasedi(PaymentPortal):
    ...
    def rasedi_return(self, **data):
        ...
        # Same as webhook: trigger payment finalization synchronously so the invoice
        # is reconciled immediately without waiting for the cron job.
        if tx:
            try:
                tx = tx.sudo()
    ...
    def rasedi_webhook(self, **data):
        ...
        # In Odoo 16, account.payment creation runs via cron (_cron_finalize_post_processing).
